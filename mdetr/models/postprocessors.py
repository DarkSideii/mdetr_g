...
class PostProcessPhrasecut(nn.Module):
    ...
    Note: this can still consume masks if they exist in `results`, but the segmentation
    postprocessor itself has been removed (no PostProcessSegm in this file).
    """
    ...
    def forward(self, results):
        """Perform the computation
        Args:
            results: Dictionary/list that comes from PostProcess (and could include masks if added elsewhere)
        """
        final_results = []
        for elem in results:
