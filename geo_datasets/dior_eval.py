...
class DiorEvaluator(CocoEvaluator):
    """
    COCO evaluator with a class-agnostic export option for DIOR.

    If use_cats=False (default), every detection is exported with category_id=1.
    This avoids pycocotools silently dropping predictions when labels are token
    indices (as in MDETR’s token-based classification) and supports class-agnostic AP.
    """
