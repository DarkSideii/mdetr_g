        ...
        for a in rec["annotations"]:
            rows.append(
                dict(
                    ...
                    phrase=a.get("phrase", ""),  # phrase we attempted to align against
                    iscrowd=a["iscrowd"],
...
class DotaModulatedDetection(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if rec["orig_size"] == (0, 0):  # just in case
    ...
    def _get_s3(self):
        if getattr(self, "_s3", None) is None:
            # New session/client in this process
    ...
    def _build_annotations(self) -> Dict[int, Dict]:
        ...
        """
        # 1) Load captions from CSV (expects 'Image' and 'Description' headers)
            ...
            for ln in raw_txt.splitlines():
                ...
                rec["annotations"].append(
                    dict(
                        ...
                        phrase=cls_raw,  # will align to ship/ships or plane/planes
                        iscrowd=0,
                    )
                )
    ...
    def _augment_captions_once(self, ann, only_ids=None):
        ...
        def _exp(batch):
            ...
            for img_id, cap in zip(batch["img_id"], batch["caption"]):
                ...
                # 2) top-up to 5 unique sentences
...
def _dota_transforms(split: str):
    """
    RP-style online augmentations:
    - Train: H/V flip, 90° rotate, color jitter (0.30), Gaussian blur (0.10),
             multi-scale resize [512..704], max_size=704, then ToTensor+Normalize.
    - Val:   resize to 704 (shorter side), then ToTensor+Normalize.
    """
    normalize = T.Compose(
        [
            T.ToTensor(),
...
class _ConvertDotaToTarget:
    ...
    def __call__(self, image, record: Dict):
        ...
        # Positive map: align each kept box to ship/ships or plane/planes in the caption
